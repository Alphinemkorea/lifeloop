...
import { PlusCircle, Users, Hash, LogOut, Sparkles, TreePine, BarChart2, ShieldCheck, Copy, Check, Share2 } from 'lucide-react';
  ...
  return (
    <div className="space-y-6 max-w-6xl mx-auto pb-12 text-slate-800">
      ...
      <div className="bg-white border border-slate-200 rounded-3xl overflow-hidden shadow-xs relative">
        <div className="h-36 md:h-48 bg-gradient-to-r from-blue-900 via-indigo-900 to-slate-900 relative">
          {space.cover_url && (
            <img src={space.cover_url} alt={space.name} className="w-full h-full object-cover opacity-30" />
          )}
        </div>

        <div className="p-6 md:p-8 flex flex-col md:flex-row items-start md:items-center justify-between gap-6 -mt-12 relative z-10">
          <div className="flex items-start gap-4">
            <div className="w-20 h-20 rounded-3xl bg-white border-4 border-white shadow-lg flex items-center justify-center text-4xl shrink-0">
              {space.icon}
            </div>
            <div className="space-y-1">
              ...
              <p className="text-xs text-slate-600 max-w-xl leading-relaxed">{space.description}</p>
          ...
          <div className="flex items-center gap-2 shrink-0 self-end md:self-center flex-wrap">
            ...
            >
              ...
              <span>Post to {space.name}</span>
            </button>

            {space.is_member && space.user_role !== 'owner' && (
              <button
        ...
        <div className="px-6 border-t border-slate-100 flex items-center gap-6 text-xs font-bold text-slate-500">
          ...
          >
            ...
            <span>Members ({space.members?.length || space.member_count})</span>
          </button>
        </div>
      </div>
        ...
        <div className="bg-white border border-slate-200 rounded-3xl p-6 shadow-xs space-y-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {space.members?.map((member) => (
              <div key={member.id} className="flex items-center gap-3 p-3 bg-slate-50 rounded-2xl border border-slate-200">
                <img
                ...
                <div className="min-w-0 flex-1">
                  <div className="flex items-center justify-between">
                    <span className="font-bold text-xs text-blue-950 truncate">{member.nickname_in_space || member.user_name}</span>
                    <span className="text-[10px] font-bold px-2 py-0.5 rounded-full bg-blue-100 text-blue-900 uppercase">
                      {member.role}
                    </span>
                  </div>
